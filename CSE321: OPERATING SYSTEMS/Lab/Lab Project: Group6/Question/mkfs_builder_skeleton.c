...
int main(int argc, char *argv[]) {
    ...
    inode_t* root_ino = (inode_t*)(disk_image + (sb->inode_table_start * BS));
    ...
    root_ino->mtime = now;
    root_ino->ctime = now;
    root_ino->direct[0] = sb->data_region_start; // absolute block number
    for(int i=1; i<DIRECT_MAX; ++i) root_ino->direct[i] = 0;
    root_ino->reserved_0 = root_ino->reserved_1 = root_ino->reserved_2 = 0;
    root_ino->proj_id = 0;
